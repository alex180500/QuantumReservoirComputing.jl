...
function local_measure(
    ...
    local_meas = Vector{Float64}(undef, length(qubits))
    return local_measure!(local_meas, ψ, meas, qubits, n_sys)
end

function local_measure(
    ρ::AbstractMatrix{T}, meas::Function=avg_z, n_sys::Integer=get_nsys(ρ)
) where {T<:Number}
    return local_measure(ρ, meas, QubitSpec(n_sys), n_sys)
end

function local_measure(
    ψ::AbstractVector{T}, meas::Function=avg_z, n_sys::Integer=get_nsys(ψ)
) where {T<:Number}
    return local_measure(ψ, meas, QubitSpec(n_sys), n_sys)
end
...
function montecarlo_measure(
    ...
    return montecarlo_measure(state_probs, n_sys; n_samples)
end
...
function measure(
    ...
    state_probs = get_probabilities(state)::Vector{real(T)}
    return measure(state_probs, n_sys; n_samples)
end

function measure(
    state_probs::AbstractVector{T}, n_samples::Integer=1_000_000
) where {T<:Real}
    outcomes = zeros(Float64, length(state_probs))
    return measure!(outcomes, state_probs, n_samples)
end

function measure!(
    outcomes::AbstractVector{S}, state_probs::AbstractVector{T}, n_samples::Integer
) where {S<:AbstractFloat,T<:Real}
    counts = Vector{Int}(undef, length(state_probs))
    return measure!(outcomes, counts, state_probs, n_samples)
end
