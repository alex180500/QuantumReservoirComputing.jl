...
function local_measure(
    ρ::AbstractMatrix{T}, meas::Function, qubits::NTuple{N,Int}, n_sys::Integer=get_nsys(ρ)
) where {N,T<:Number}
...
function montecarlo_measure(
    ρ::AbstractMatrix{T}, n_sys::Integer=get_nsys(ρ); n_samples::Integer=1_000_000
) where {T<:Number}
...
function simulated_measure(
    ρ::AbstractMatrix{T}, n_samples::Integer=1_000_000
) where {T<:Number}
