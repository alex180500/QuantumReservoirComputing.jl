...
function vn_entropy(all_λ::AbstractVector{T}; tol::Real=eps(real(T))) where {T<:Real}
    S = 0.0
...
function vn_entropy(ρ::AbstractMatrix{T}; tol::Real=eps(real(T))) where {T<:Number}
    all_λ = eigvals(Hermitian(ρ))
    return vn_entropy(all_λ; tol)
end

# calculates the mutual information of a bipartite density matrix
function mutual_info(ρ::AbstractMatrix{T}; tol::Real=eps(real(T))) where {T<:Number}
